...
message.react(doru)
client.channels.cache.get(BanLog).send(new MessageEmbed().setTitle(`Bir Üye Sunucudan Yasaklandı !`).setColor(inventory.Colors.Special).setFooter(message.author.tag, message.author.avatarURL({dynamic:true})).setTimestamp().setDescription(`Bir üye sunucudan yasaklandı, **${kullanici.user.tag}** adlı kullanıcı \`${sebep}\` sebebiyle sunucudan yasaklandı. ${inventory.Emoji.True}\n\nYetkili: ${message.author} (\`${message.author.id}\`)\nKullanıcı: ${kullanici.user} (\`${kullanici.user.id}\`)\nSebep: \`${sebep}\` \n Jail Atılma Saati: \`${banatılma}\` \n Tarih: \`${moment(Date.now()).add(10,"hours").format("HH:mm:ss DD MMMM YYYY")}\``));

}

exports.conf = {aliases: ["ban", "yasakla", "yargı", "allahinakavustur"]}
